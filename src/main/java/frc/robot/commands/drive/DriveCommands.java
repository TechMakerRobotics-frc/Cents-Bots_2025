...
public class DriveCommands {
  ...
  private static final double TIPPING_THRESHOLD = 3.0;
  ...
  public static Command joystickDriveWithBalance(
      Drive drive,
      DoubleSupplier xSupplier,
      DoubleSupplier ySupplier,
      DoubleSupplier omegaSupplier) {
    ...
    return Commands.run(
        () -> {
          // Obtém a entrada translacional do joystick
          Translation2d joystickInput =
              getLinearVelocityFromJoysticks(
                  xLimiter.calculate(xSupplier.getAsDouble()), ySupplier.getAsDouble());

          // Lê os valores do IMU (pitch e roll, em graus)
          double pitch = drive.getPitchDegrees();
          double roll = drive.getRollDegrees();

          // Verifica se a inclinação ultrapassa o limiar
          if (Math.abs(pitch) > TIPPING_THRESHOLD || Math.abs(roll) > TIPPING_THRESHOLD) {
            // Calcula o ângulo da inclinação.
            // Alteração: inverter os sinais para que a correção seja na mesma direção da queda.
            double tiltAngleRadians = Math.atan2(-roll, -pitch);
            Rotation2d tiltDirection = new Rotation2d(tiltAngleRadians);

            // Usa o vetor unitário (0, 1) – conforme seu código original – e o rotaciona
            // para que a correção seja aplicada na direção da queda.
            joystickInput = new Translation2d(0, 1).rotateBy(tiltDirection);
          }

          // Converte o vetor resultante em velocidade linear (m/s) escalada para a velocidade
          // máxima
          Translation2d linearVelocity =
              new Translation2d(
          ...
          // Processa a entrada de rotação do joystick
          double omega = MathUtil.applyDeadband(omegaSupplier.getAsDouble(), DEADBAND);
          omega = Math.copySign(omega * omega, omega);
          omega *= drive.getMaxAngularSpeedRadPerSec();

          // Converte para ChassisSpeeds – se estiver fora do limiar, usa a conversão field–relative
          boolean isTipping =
              (Math.abs(pitch) > TIPPING_THRESHOLD || Math.abs(roll) > TIPPING_THRESHOLD);
          ChassisSpeeds speeds;
          if (isTipping) {
            // Quando há correção, aplica as velocidades no referencial do robô
            speeds = new ChassisSpeeds(-linearVelocity.getX(), linearVelocity.getY(), omega);
          } else {
            // Caso contrário, usa as velocidades field–relative
            boolean isFlipped =
                DriverStation.getAlliance().isPresent()
                    && DriverStation.getAlliance().get() == Alliance.Red;
            ...
            speeds =
                ChassisSpeeds.fromFieldRelativeSpeeds(
                    linearVelocity.getX(), linearVelocity.getY(), omega, currentRotation);
          }

          // Comanda o drivetrain com as velocidades calculadas
          drive.runVelocity(speeds);
        },
        drive);
  }
