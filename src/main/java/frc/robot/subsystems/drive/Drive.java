...
public class Drive extends SubsystemBase implements Vision.VisionConsumer {
  ...
  static final double ODOMETRY_FREQUENCY =
      new CANBus(DriveConstants.CANBUS).isNetworkFD() ? 250.0 : 100.0;
  public static final double DRIVE_BASE_RADIUS =
      Math.max(
          Math.max(
              Math.hypot(
                  DriveConstants.X_DISTANCE_FL.in(Meter), DriveConstants.X_DISTANCE_FL.in(Meter)),
              Math.hypot(
                  DriveConstants.X_DISTANCE_FR.in(Meter), DriveConstants.X_DISTANCE_FR.in(Meter))),
          Math.max(
              Math.hypot(
                  DriveConstants.X_DISTANCE_BL.in(Meter), DriveConstants.X_DISTANCE_BL.in(Meter)),
              Math.hypot(
                  DriveConstants.X_DISTANCE_BR.in(Meter), DriveConstants.X_DISTANCE_BR.in(Meter))));

  private LoggedNetworkNumber kp = new LoggedNetworkNumber("/Tuning/P - Turn", 0);
  private LoggedNetworkNumber ki = new LoggedNetworkNumber("/Tuning/I - Turn", 0);
  private LoggedNetworkNumber kd = new LoggedNetworkNumber("/Tuning/D - Turn", 0);
  private LoggedNetworkNumber kf = new LoggedNetworkNumber("/Tuning/F - Turn", 0);
  ...
  public void periodic() {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (gyroInputs.connected) {
        ...
        pitchDegrees = gyroInputs.pitchPositionDegrees;
        rollDegrees = gyroInputs.rollPositionDegrees;
      } else {
  ...
  public void runVelocity(ChassisSpeeds speeds) {
    ...
    SwerveDriveKinematics.desaturateWheelSpeeds(setpointStates, DriveConstants.MAX_SPEED_MTS_SEC);
  ...
  public double getMaxLinearSpeedMetersPerSec() {
    return DriveConstants.MAX_SPEED_MTS_SEC;
  }
  ...
  public void updatePID() {
    for (int i = 0; i < 4; i++) {
      modules[i].setPIDF(kp.get(), ki.get(), kd.get(), kf.get());
    }
  }
