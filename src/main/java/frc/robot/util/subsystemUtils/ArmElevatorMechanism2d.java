...
/**
 * A Mechanism2d object to display the current angle and target angle, and the current position and
 * target position of an arm elevator.
 */
public class ArmElevatorMechanism2d {
  private final String key;
  private final LoggedMechanism2d mechanism;
  private final LoggedMechanismLigament2d currentPositionLigament, targetPositionLigament;
   ...
   */
  public ArmElevatorMechanism2d(
      String name, double maximumLength, double minimumLength, Color mechanismColor) {
    ...
    this.currentPositionLigament =
        root.append(
            new LoggedMechanismLigament2d(
                "ZCurrentPositionLigament",
                0,
                0,
                MechanismConstants.MECHANISM_LINE_WIDTH,
                new Color8Bit(mechanismColor)));
    this.targetPositionLigament =
        root.append(
            new LoggedMechanismLigament2d(
                "TargetPositionLigament",
                0,
                0,
                MechanismConstants.TARGET_ELEVATOR_POSITION_LIGAMENT_WIDTH,
                MechanismConstants.GRAY));
  }

  /**
   * Updates the mechanism's position and target position as well as the mechanism's angle and
   * target angle, then logs the Mechanism2d object.
   *
