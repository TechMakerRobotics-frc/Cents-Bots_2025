...
/**
 * A class that represents a {@link Translation3d} that can be flipped when the robot is on the red
 * alliance. The Z value will have no change.
 */
public class FlippableTranslation3d extends Flippable<Translation3d> {
   ...
   * @param shouldFlipWhenRedAlliance should the position be flipped when the robot is on the red
   *     alliance
   */
   ...
   *     alliance
   */
  public FlippableTranslation3d(
      Translation3d nonFlippedTranslation, boolean shouldFlipWhenRedAlliance) {
    super(nonFlippedTranslation, shouldFlipWhenRedAlliance);
  }

  /**
   * Flips the given translation. The translation will be flipped if the robot is on the red
   * alliance and {@link #shouldFlipWhenRedAlliance} is true. The Z value will have no change.
   *
  ...
  protected Translation3d flip(Translation3d translation) {
    final Translation2d flippedTranslation2d =
        FlippingUtil.flipFieldPosition(translation.toTranslation2d());
    return new Translation3d(
        flippedTranslation2d.getX(), flippedTranslation2d.getY(), translation.getZ());
  }
}
