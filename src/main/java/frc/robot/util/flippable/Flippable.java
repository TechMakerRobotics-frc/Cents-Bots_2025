package frc.robot.util.flippable;
...
/**
 * A class that allows for objects to be flipped across the center of the field when the robot is on
 * the red alliance. This is useful for placing field elements and other objects that are flipped
 * across the field, or for flipping the target heading to face a field element. This either
 * represents a field that is mirrored vertically over the center of the field, or a field that is
 * rotationally symmetric: the red alliance side is the blue alliance side rotated by 180 degrees.
 * The code will automatically determine which type of field the robot is on and flip the object
 * accordingly.
 *
...
public abstract class Flippable<T> {
  ...
  /**
   * Gets a command that updates the current alliance. This is used to cache the alliance every 0.5
   * seconds. Ignoring disable is used to update the current alliance when the robot is disabled.
   *
   * @return the command
   */
  private static Command getUpdateAllianceCommand() {
    return new InstantCommand(() -> IS_RED_ALLIANCE = notCachedIsRedAlliance())
   ...
   *
   * @param nonFlippedObject the object when the robot is on the blue alliance; the non-flipped
   *     object
   * @param shouldFlipWhenRedAlliance should the object should be flipped when the robot is on the
   *     red alliance
   */
  ...
  /**
   * If the robot is on the red alliance and the object should be flipped, the flipped object is
   * returned. Otherwise, the non-flipped object is returned.
   *
